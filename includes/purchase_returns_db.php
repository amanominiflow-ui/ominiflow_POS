...
function create_purchase_return(int $poId, array $items, string $refundMethod = 'vendor_credit', string $notes = '', ?int $userId = null): array {
    $db = get_db();

    try {
        $db->beginTransaction();

        $stmtPO = $db->prepare('SELECT * FROM purchase_orders WHERE id = :id LIMIT 1');
        ...
        $stmtCount = $db->query("SELECT COUNT(*) FROM purchase_returns WHERE DATE(created_at) = CURDATE()");
        ...
        $stmtPR = $db->prepare('
            INSERT INTO purchase_returns (return_number, purchase_order_id, vendor_id, user_id, total_amount, refund_method, status, notes, created_at, updated_at)
            VALUES (:rnum, :poid, :vid, :uid, :total, :method, "completed", :notes, NOW(), NOW())
        ');
        $stmtPR->execute([
            'rnum' => $returnNumber,
            'poid' => $poId,
        ...
        $stmtVenAdj = $db->prepare('UPDATE vendors SET outstanding_balance = GREATEST(0, outstanding_balance - :amt) WHERE id = :id');
        $stmtVenAdj->execute(['amt' => $totalReturnAmount, 'id' => $vendorId]);
...
function record_vendor_payment(int $vendorId, float $amount, ?int $poId = null, string $method = 'bank_transfer', string $ref = '', string $notes = '', ?int $userId = null): array {
    $db = get_db();

    try {
        $db->beginTransaction();

        $stmtCount = $db->query("SELECT COUNT(*) FROM vendor_payments WHERE DATE(created_at) = CURDATE()");
        $seq = (int)$stmtCount->fetchColumn() + 1;
        $paymentNumber = 'VPAY-' . date('Ymd') . '-' . str_pad((string)$seq, 4, '0', STR_PAD_LEFT);

        $stmtVP = $db->prepare('
            INSERT INTO vendor_payments (payment_number, vendor_id, purchase_order_id, user_id, amount, payment_method, transaction_ref, notes, status, created_at)
            VALUES (:pnum, :vid, :poid, :uid, :amt, :method, :ref, :notes, "paid", NOW())
        ');
        $stmtVP->execute([
            'pnum' => $paymentNumber,
            'vid' => $vendorId,
        ...
        $stmtVen = $db->prepare('UPDATE vendors SET outstanding_balance = GREATEST(0, outstanding_balance - :amt) WHERE id = :id');
        $stmtVen->execute(['amt' => $amount, 'id' => $vendorId]);
...
function get_purchase_returns(int $limit = 50): array {
    $db = get_db();
    $stmt = $db->prepare('
